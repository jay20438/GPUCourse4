        ...
        for city in os.listdir(input_split_dir):
            ...
            for file in os.listdir(input_city_dir):
                if file.endswith(".png"):
                    ...
                    # Perform downsampling after blur
                    low_res_img = cv2.resize(blurred_img, (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_LINEAR)
                    
                    output_path = os.path.join(output_city_dir, file)
                    cv2.imwrite(output_path, low_res_img)
                    print(f"Processed {output_path}")

input_dir = "filtered_data/leftImg8bit_trainvaltest"
output_dir = "filtered_data/leftImg8bit_low_res_gaussian_blur"
